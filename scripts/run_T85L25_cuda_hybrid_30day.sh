...
LOG="${GFDL_BASE}/logs/T85L25_hybrid_cuda_30day.log"
mkdir -p "${GFDL_BASE}/logs"
exec > >(tee "${LOG}") 2>&1

echo "=== T85L25 CUDA hybrid forcing 30-day run ==="
...
time python3 scripts/run_T85L25_case.py \
  --exp-name held_suarez_hybrid_cuda_T85L25 \
  --executable-name held_suarez_hybrid.x \
  --backend-label cuda_hybrid \
  --resolution T85 \
  --levels 25 \
  --dt-atmos 300 \
  --days 30 \
