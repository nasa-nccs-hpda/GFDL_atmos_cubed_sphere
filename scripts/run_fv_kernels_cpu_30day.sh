...
export FV_KERNELS_CPU_EXP_NAME="${FV_KERNELS_CPU_EXP_NAME:-held_suarez_fv_kernels_30day}"
...
echo "=== Held-Suarez fv_advection kernel-bundle CPU C++ 30-day run ==="
...
echo "executable=held_suarez_fv_kernels.x"
echo "experiment=${FV_KERNELS_CPU_EXP_NAME}"
...
time python3 hybrid_experiments/held_suarez_cpp_force/run_hybrid_held_suarez.py \
  ...
  --exp-name '${FV_KERNELS_CPU_EXP_NAME}' \
  --days 30 \
  --production-diag \
