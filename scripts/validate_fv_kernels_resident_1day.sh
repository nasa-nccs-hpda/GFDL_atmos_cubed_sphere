...
LOG="${GFDL_BASE}/logs/fv_kernels_cuda_resident_1day_validation.log"
...
python3 tests/validate_T85L25_forcing_outputs.py \
  ...
  --cuda-exp held_suarez_fv_kernels_cuda_resident_1day \
  --run 1 \
  --filename atmos_monthly.nc \
  --data-root "${GFDL_DATA}" \
  --markdown-out tests/reports/fv_advection_kernels_resident_1day_model_validation.md \
  --json-out tests/reports/fv_advection_kernels_resident_1day_model_validation.json \
  2>&1 | tee "${LOG}"
