...
LOG="${GFDL_BASE}/logs/fv_semi_y_cuda_30day.log"
mkdir -p "${GFDL_BASE}/logs"
exec > >(tee "${LOG}") 2>&1

echo "=== Held-Suarez semi_y_3d CUDA overlay 30-day run ==="
...
echo "executable=held_suarez_fv_semi_y_3d_cuda.x"
echo "experiment=held_suarez_fv_semi_y_3d_cuda_30day"
...
time python3 hybrid_experiments/held_suarez_cpp_force/run_hybrid_held_suarez.py \
  --executable-name held_suarez_fv_semi_y_3d_cuda.x \
  --exp-name held_suarez_fv_semi_y_3d_cuda_30day \
  --days 30 \
  --production-diag \
  --num-cores 16 \
  ${overwrite_arg[*]}
