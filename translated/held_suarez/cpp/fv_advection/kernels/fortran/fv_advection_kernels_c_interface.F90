module fv_advection_kernels_c_interface
  ...
  interface
    ...
    function fv_advection_resident_begin_cuda_c(nx, js, je, nz, half_dt, dx, fold_div, &
      ...
      use, intrinsic :: iso_c_binding, only: c_double, c_int, c_ptr
      integer(c_int), value :: nx, js, je, nz, fold_div
      real(c_double), value :: half_dt, dx
      type(c_ptr), value :: c, cc, dy, dy_plus, dy_minus, dyy, ua, q, va, q1
...
contains
  ...
  subroutine fv_advection_resident_begin_wrapper(nx, js, je, nz, half_dt, dx, fold_div, &
    ...
    real(c_double), intent(in), target :: ua(nx,js:je,nz), q(nx,js-2:je+2,nz), va(nx,js-2:je+2,nz)
    real(c_double), intent(out), target :: q1(nx,js:je,nz)
    integer, intent(out) :: ierr
    ierr = int(fv_advection_resident_begin_cuda_c(int(nx,c_int), int(js,c_int), &
      int(je,c_int), int(nz,c_int), half_dt, dx, monotone_flag(fold_div), &
      c_loc(c), c_loc(cc), c_loc(dy), c_loc(dy_plus), c_loc(dy_minus), c_loc(dyy), &
      c_loc(ua), c_loc(q), c_loc(va), c_loc(q1)))
