module fv_advection_kernels_c_interface
  ...
  interface
    ...
    function fv_advection_resident_begin_cuda_c(nx, js, je, nz, half_dt, dx, c, ua, q, q1, va, dyy) &
        bind(C, name='fv_advection_resident_begin_cuda_c')
      use, intrinsic :: iso_c_binding, only: c_double, c_int, c_ptr
      integer(c_int), value :: nx, js, je, nz
      real(c_double), value :: half_dt, dx
      type(c_ptr), value :: c, ua, q, q1, va, dyy
...
contains
  ...
  subroutine fv_advection_resident_begin_wrapper(nx, js, je, nz, half_dt, dx, c, ua, q, q1, va, dyy, ierr)
    ...
    ierr = int(fv_advection_resident_begin_cuda_c(int(nx,c_int), int(js,c_int), &
      int(je,c_int), int(nz,c_int), half_dt, dx, c_loc(c), c_loc(ua), c_loc(q), &
      c_loc(q1), c_loc(va), c_loc(dyy)))
