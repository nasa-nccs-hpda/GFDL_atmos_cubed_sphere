...
! for cfl diagnostics with finite volume schemes
  ...
  real :: profile_vert_advection_seconds = 0.0
  integer :: profile_vert_advection_calls = 0
#endif
...
!   size(w,3) == size(dz,3)+1 == size(r,3)+1 == size(rdt,3)+1 == size(mask,3)+1
 ...
 if(profile_vert_advection_rate > 0) then
   profile_vert_advection_seconds = profile_vert_advection_seconds + &
        real(profile_vert_advection_t1 - profile_vert_advection_t0)/real(profile_vert_advection_rate)
 endif
 profile_vert_advection_calls = profile_vert_advection_calls + 1
#endif
...
!-------------------------------------------------------------------------------
 ...
 subroutine vert_advection_end
  ...
  integer :: outunit
#ifdef PROFILE_VERT_ADVECTION
  real :: profile_vert_advection_max_seconds, profile_vert_advection_avg_seconds
  integer :: profile_vert_advection_max_calls
#endif

#ifdef PROFILE_VERT_ADVECTION
    profile_vert_advection_max_seconds = profile_vert_advection_seconds
    profile_vert_advection_max_calls = profile_vert_advection_calls
    call mpp_max(profile_vert_advection_max_seconds)
    call mpp_max(profile_vert_advection_max_calls)
    profile_vert_advection_avg_seconds = 0.0
    if(profile_vert_advection_max_calls > 0) then
      profile_vert_advection_avg_seconds = profile_vert_advection_max_seconds / real(profile_vert_advection_max_calls)
    endif
    if(mpp_pe() == 0) then
      outunit = stdout()
      write(outunit,*) 'PROFILE_VERT_ADVECTION calls_max=', profile_vert_advection_max_calls, &
                       ' seconds_max=', profile_vert_advection_max_seconds, &
                       ' avg_seconds_per_call_max=', profile_vert_advection_avg_seconds
    endif
#endif
