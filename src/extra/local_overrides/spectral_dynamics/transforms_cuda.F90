...
#ifdef USE_CUDA_TRANSFORMS
interface
   integer(c_int) function transforms_horizontal_advection_cuda_c( &
        u_grid, v_grid, cosm_lat, dx_grid, dy_grid, tendency, ni, nj, nk) &
        bind(C, name="transforms_horizontal_advection_cuda_c")
      import :: c_double, c_int
      real(c_double), intent(in) :: u_grid(*), v_grid(*), cosm_lat(*)
      real(c_double), intent(inout) :: dx_grid(*), dy_grid(*), tendency(*)
      integer(c_int), value :: ni, nj, nk
   end function transforms_horizontal_advection_cuda_c

   integer(c_int) function transforms_divide_two_by_cos_cuda_c( &
        a_grid, b_grid, cosm_lat, ni, nj, nk) &
        bind(C, name="transforms_divide_two_by_cos_cuda_c")
      import :: c_double, c_int
      real(c_double), intent(inout) :: a_grid(*), b_grid(*)
      real(c_double), intent(in) :: cosm_lat(*)
      integer(c_int), value :: ni, nj, nk
   end function transforms_divide_two_by_cos_cuda_c
end interface
#endif
...
call divide_two_by_cos_fused    (u_grid, v_grid)
...
call horizontal_advection_accumulate_fused(u_grid, v_grid, dx_grid, dy_grid, tendency)

return
end subroutine horizontal_advection_3d

!-------------------------------------------------------------------------
subroutine divide_two_by_cos_fused(a_grid, b_grid)
!-------------------------------------------------------------------------

real, intent (inout), dimension(:,:,:) :: a_grid, b_grid

real, dimension(size(a_grid,2)) :: cosm_lat
integer :: i, j, k, ierr

if(.not.module_is_initialized) then
  call error_mesg('divide_two_by_cos_fused','transforms module is not initialized', FATAL)
end if

call get_cosm_lat(cosm_lat)

#ifdef USE_CUDA_TRANSFORMS
if(transforms_cuda_runtime_enabled() .and. &
   size(a_grid,1) == size(b_grid,1) .and. size(a_grid,2) == size(b_grid,2) .and. size(a_grid,3) == size(b_grid,3)) then
  ierr = transforms_divide_two_by_cos_cuda_c(a_grid, b_grid, cosm_lat, size(a_grid,1), size(a_grid,2), size(a_grid,3))
  if(ierr == 0) return
endif
#endif

do k=1,size(a_grid,3)
  do j=1,size(a_grid,2)
    do i=1,size(a_grid,1)
      a_grid(i,j,k)=a_grid(i,j,k)*cosm_lat(j)
      b_grid(i,j,k)=b_grid(i,j,k)*cosm_lat(j)
    end do
  end do
end do

return
end subroutine divide_two_by_cos_fused

!-------------------------------------------------------------------------
subroutine horizontal_advection_accumulate_fused(u_grid, v_grid, dx_grid, dy_grid, tendency)
!-------------------------------------------------------------------------

real, intent (in),    dimension(:,:,:) :: u_grid, v_grid
real, intent (inout), dimension(:,:,:) :: dx_grid, dy_grid
real, intent (inout), dimension(:,:,:) :: tendency

real, dimension(size(dx_grid,2)) :: cosm_lat
integer :: i, j, k, ierr
real :: dx, dy

if(.not.module_is_initialized) then
  call error_mesg('horizontal_advection_accumulate_fused','transforms module is not initialized', FATAL)
end if
...
#ifdef USE_CUDA_TRANSFORMS
if(transforms_cuda_runtime_enabled() .and. &
   size(u_grid,1) == size(dx_grid,1) .and. size(u_grid,2) == size(dx_grid,2) .and. size(u_grid,3) == size(dx_grid,3)) then
  ierr = transforms_horizontal_advection_cuda_c(u_grid, v_grid, cosm_lat, dx_grid, dy_grid, tendency, &
       size(dx_grid,1), size(dx_grid,2), size(dx_grid,3))
  if(ierr == 0) return
endif
#endif

do k=1,size(dx_grid,3)
  do j=1,size(dx_grid,2)
    do i=1,size(dx_grid,1)
      dx = dx_grid(i,j,k)*cosm_lat(j)
      dy = dy_grid(i,j,k)*cosm_lat(j)
      dx_grid(i,j,k) = dx
      dy_grid(i,j,k) = dy
      tendency(i,j,k) = tendency(i,j,k) - u_grid(i,j,k)*dx - v_grid(i,j,k)*dy
    end do
  end do
end do

return
end subroutine horizontal_advection_accumulate_fused

#ifdef USE_CUDA_TRANSFORMS
!-------------------------------------------------------------------------
logical function transforms_cuda_runtime_enabled()
!-------------------------------------------------------------------------

character(len=32) :: value
integer :: status

transforms_cuda_runtime_enabled = .false.

call get_environment_variable('TRANSFORMS_BACKEND', value, status=status)
if(status == 0) then
  transforms_cuda_runtime_enabled = trim(value) == 'cuda'
  return
endif

call get_environment_variable('HS_FORCE_BACKEND', value, status=status)
if(status == 0) then
  transforms_cuda_runtime_enabled = trim(value) == 'cuda'
endif

return
end function transforms_cuda_runtime_enabled
#endif
