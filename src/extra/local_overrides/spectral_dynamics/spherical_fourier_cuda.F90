...
#ifdef USE_CUDA_SPHERICAL_FOURIER
interface
   integer(c_int) function spherical_fourier_s2f_cuda_c( &
        spherical, fourier, legendre, jstart, nm, nn, nk, nj, nd, ns, ne, neven, nodd, south_to_north) &
        bind(C, name="spherical_fourier_s2f_cuda_c")
      import :: c_double, c_double_complex, c_int
      complex(c_double_complex), intent(in) :: spherical(*)
      complex(c_double_complex), intent(out) :: fourier(*)
      real(c_double), intent(in) :: legendre(*)
      integer(c_int), intent(in) :: jstart(*)
      integer(c_int), value :: nm, nn, nk, nj, nd, ns, ne, neven, nodd, south_to_north
   end function spherical_fourier_s2f_cuda_c

   integer(c_int) function spherical_fourier_f2s_cuda_c( &
        fourier, spherical, legendre_wts, jstart, nm, nn, nk, nj, nd, ns, ne, neven, nodd, south_to_north) &
        bind(C, name="spherical_fourier_f2s_cuda_c")
      import :: c_double, c_double_complex, c_int
      complex(c_double_complex), intent(in) :: fourier(*)
      complex(c_double_complex), intent(out) :: spherical(*)
      real(c_double), intent(in) :: legendre_wts(*)
      integer(c_int), intent(in) :: jstart(*)
      integer(c_int), value :: nm, nn, nk, nj, nd, ns, ne, neven, nodd, south_to_north
   end function spherical_fourier_f2s_cuda_c
end interface
#endif
...
!in parallel, assumes that computational domains are being passed in.
  ...
  integer :: cuda_ierr, south_to_north_int
...
#ifdef USE_CUDA_SPHERICAL_FOURIER
  if(spherical_fourier_cuda_runtime_enabled()) then
     if(south_to_north_local) then
        south_to_north_int = 1
     else
        south_to_north_int = 0
     endif
     cuda_ierr = spherical_fourier_s2f_cuda_c( &
          spherical, fourier, legendre, jstart, &
          size(spherical,1), size(spherical,2), size(spherical,3), &
          size(fourier,2), size(fourier,4), ns, ne, neven, nodd, south_to_north_int)
     if(cuda_ierr == 0) return
  endif
#endif
...
!  complex, dimension(ms:me, size(spherical,3)) :: x_odd, x_even
  ...
  if(spherical_fourier_cuda_runtime_enabled()) then
     ...
     cuda_ierr = spherical_fourier_f2s_cuda_c( &
          fourier, spherical, legendre_wts, jstart, &
          size(spherical,1), size(spherical,2), size(spherical,3), &
          size(fourier,2), size(fourier,4), ns, ne, neven, nodd, south_to_north_int)
     if(cuda_ierr == 0) return
  endif
...
!----------------------------------------------------------------------
#ifdef USE_CUDA_SPHERICAL_FOURIER
logical function spherical_fourier_cuda_runtime_enabled()

character(len=32) :: value
integer :: status

spherical_fourier_cuda_runtime_enabled = .false.

call get_environment_variable('SPHERICAL_FOURIER_BACKEND', value, status=status)
if(status == 0) then
  spherical_fourier_cuda_runtime_enabled = trim(value) == 'cuda'
  return
endif
...
return
end function spherical_fourier_cuda_runtime_enabled
!----------------------------------------------------------------------
#endif
