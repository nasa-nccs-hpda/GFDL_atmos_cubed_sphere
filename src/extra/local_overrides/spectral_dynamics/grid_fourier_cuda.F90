...
#ifdef USE_CUDA_GRID_FOURIER
interface
   integer(c_int) function grid_fourier_r2c_cuda_c(grid, fourier, n, batch) bind(C, name="grid_fourier_r2c_cuda_c")
   ...
   end function grid_fourier_r2c_cuda_c

   integer(c_int) function grid_fourier_c2r_cuda_c(fourier, grid, n, batch) bind(C, name="grid_fourier_c2r_cuda_c")
      import :: c_double, c_double_complex, c_int
      complex(c_double_complex), intent(in) :: fourier(*)
      real(c_double), intent(out) :: grid(*)
      integer(c_int), value :: n, batch
   end function grid_fourier_c2r_cuda_c
end interface
#endif
...
#ifdef USE_CUDA_GRID_FOURIER
if(grid_fourier_cuda_runtime_enabled()) then
  cuda_ierr = grid_fourier_r2c_cuda_c(grid1, fourier, size(grid,1), size(grid,2)*size(grid,3))
...
#ifdef USE_CUDA_GRID_FOURIER
if(grid_fourier_cuda_runtime_enabled()) then
  cuda_ierr = grid_fourier_c2r_cuda_c(fourier, grid1, 2*(size(fourier,1)-1), size(fourier,2)*size(fourier,3))
  if(cuda_ierr == 0) then
...
#ifdef USE_CUDA_GRID_FOURIER
logical function grid_fourier_cuda_runtime_enabled()

character(len=32) :: value
integer :: status

grid_fourier_cuda_runtime_enabled = .false.

call get_environment_variable('GRID_FOURIER_BACKEND', value, status=status)
if(status == 0) then
  grid_fourier_cuda_runtime_enabled = trim(value) == 'cuda'
  return
endif
...
return
end function grid_fourier_cuda_runtime_enabled
