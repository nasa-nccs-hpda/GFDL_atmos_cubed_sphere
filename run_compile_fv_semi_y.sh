...
export USE_CUDA_SEMI_Y_3D=${USE_CUDA_SEMI_Y_3D:-0}
export NVCC=${NVCC:-nvcc}

LOG_DIR="${SCRIPT_DIR}/logs"
mkdir -p "${LOG_DIR}"
if [ "${USE_CUDA_SEMI_Y_3D}" = "1" ]; then
  TARGET=fv_semi_y_cuda
  TEMPLATE=fv_hybrid_cuda
  LOG_PREFIX=fv_semi_y_cuda_compile
else
  TARGET=fv_semi_y
  TEMPLATE=fv_hybrid
  LOG_PREFIX=fv_semi_y_compile
fi
...
export USE_CUDA_SEMI_Y_3D=${USE_CUDA_SEMI_Y_3D}
export NVCC=${NVCC}
...
echo 'USE_CUDA_SEMI_Y_3D=' \${USE_CUDA_SEMI_Y_3D}
echo 'GFDL_MKMF_TEMPLATE=' \${GFDL_MKMF_TEMPLATE}
echo 'TARGET=' ${TARGET}

cd ${GFDL_BASE}
mkdir -p logs
echo '=== Compile Held-Suarez fv semi_y_3d overlay ==='
